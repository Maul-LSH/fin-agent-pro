...
它把公开市场数据、财报交叉验证、量化风险模型、交易行为优先的市场热力图、板块级 AI 分析、带可编辑 drivers 的估值框架、投资组合诊断和整页 LLM 报告整合在一起，同时始终区分“数据真正支持了什么”和“哪些部分仍然未知”。
...
- 把估值当成“框架问题”，而不是单一答案：DCF、市场价、华尔街目标价、可编辑业务 drivers、增长、利润率和估值倍数可能同时给出不同信号
- 尽可能把 API key 和投资组合数据保存在本地

最终形成一个更接近分析师工作流的路径：

**市场背景 -> 公司基本面 -> 财报交叉验证 -> 风险模型 -> 估值框架 -> AI 解读 -> 明确不确定性 -> 可导出整页报告**
...
| **估值工作区** | 10 年两阶段 DCF，加上 WACC 构成、公司名/ticker 自动识别、估值框架分类、可编辑 driver-based SOTP 模型、市场隐含假设、估值倍数、利润率趋势和华尔街目标价共识。 |
| **可计算 Driver 模型** | 对已支持公司，不再只靠一个增长率滑块。以 Tesla 为例，可以拆成 Auto、Energy、FSD 订阅和 Robotaxi 假设，并实时计算分部收入、利润、价值、SOTP 每股价值和市场差额。 |
...
`fin-agent-pro` 不假装一个模型能告诉你公司“真正值多少钱”。估值页会先选择更合适的估值框架，再展示分歧：

```text
我们的 DCF 情景      vs.      当前市场价格      vs.      华尔街目标价
```

对高增长股票来说，这个差距本身往往就是信号。产品会把 DCF 与估值框架分类、Forward P/E、PEG、EV/Sales、EV/Revenue Growth、营收增速、盈利增速、毛利率、营业利润率、市场隐含增长率和分析师目标价共识放在一起看。

当前估值引擎包括：

- **Ticker 识别：** 输入 `Tesla`、`特斯拉`、`摩根大通` 等公司名时，系统会尽量先识别成正确的美股 ticker，再运行估值。
- **估值框架生成器：** 自动把公司映射到稳定现金流复利型、软件/云平台、AI 半导体周期、期权型增长公司、银行、REIT、医药/生物科技管线、能源/大宗商品等估值原型。
- **Driver Library：** 不同框架会暴露对应的收入、利润率、资本、概率、期权、金融行业或实物资产 drivers。
- **可计算 Driver 估值：** 第一版支持 Tesla 式 optionality SOTP，拆分 Auto、Energy、FSD 订阅和 Robotaxi。
- **市场隐含假设：** 反推出当前市场价格必须相信什么，包括所需 FCF 增速、终值增长率、相对 DCF 的市场溢价、终值依赖度，以及相关场景下的期权价值缺口。

Tesla 是当前的核心示例：简单 DCF 可能远低于市场价格，但可编辑 driver 模型会把这个差距拆成车辆交付量、ASP、Energy 增长、FSD 渗透率、月 ARPU、Robotaxi TAM、市场份额、take rate 和成功概率等明确假设。

当公司呈现高增长或高波动特征时，DCF 会被明确标记为**极不稳定**。内在价值是一个情景输出，不是定论。

分析师目标价也同样只是第二意见，而不是正确答案。当前华尔街目标价卡片来自 Yahoo Finance via yfinance，可能延迟或不完整。如果 Yahoo Finance 提供近期机构评级动作，卡片可以打开侧边栏查看可用覆盖记录；如果只有汇总共识，UI 会诚实说明，而不是伪造精确度。
...
│   │   ├── valuation.py          # DCF、估值框架、ticker 识别、driver 估值接口
│   │   └── portfolio.py          # 投资组合诊断
│   └── core/
│       ├── data.py               # 统一公司数据接入
...
│       ├── dcf.py                # WACC + 两阶段 DCF 引擎 + 市场隐含假设
│       ├── valuation_framework.py # 估值框架分类 + driver 模板
│       ├── driver_valuation.py   # 可计算 driver-based SOTP 模型
│       ├── symbols.py            # 公司名 / ticker 识别
...
5. 使用 **估值** 对比 DCF、市场价、华尔街目标价、估值倍数、增长、利润率质量、估值框架、可编辑 drivers 和市场隐含假设
...
- [x] 加入 ticker/公司名识别、估值框架生成器和 driver 模板
- [x] 支持 Tesla 的 Auto、Energy、FSD 订阅和 Robotaxi 可计算 driver 估值
- [x] 增加所需增长率、终值增长率、估值溢价和期权缺口的市场隐含假设
