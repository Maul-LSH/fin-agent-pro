...
export function AttentionQuadrant({ data, loading, onSelectSector }: Props) {
  ...
  if (loading) {
    return (
      <div className="rounded-3xl bg-slate-100 dark:bg-slate-800 h-[480px] animate-pulse" />
    );
  }

  if (bubbles.length === 0) {
    return (
      <div className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 h-[480px] flex items-center justify-center text-slate-400 dark:text-slate-500">
        {t("noData")}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {topHot.map((b, i) => (
          <motion.div
            ...
            className="px-3 py-1 rounded-full bg-gradient-to-r from-orange-100 to-rose-100 dark:from-orange-950 dark:to-rose-950 text-orange-900 dark:text-orange-100 text-xs font-medium border border-orange-200 dark:border-orange-900"
          >
            {b.label}
            {b.change_pct !== null && (
      ...
      <div className="relative rounded-3xl bg-gradient-to-br from-white via-slate-50 to-blue-50/30 dark:from-slate-900 dark:via-slate-900 dark:to-blue-950/30 border border-slate-200 dark:border-slate-800 p-8 overflow-hidden min-h-[500px]">
        <div className="absolute inset-0 pointer-events-none opacity-50">
          <div className="absolute top-1/4 left-1/3 w-64 h-64 bg-rose-300/20 dark:bg-rose-500/10 rounded-full blur-3xl" />
          <div className="absolute bottom-1/4 right-1/3 w-64 h-64 bg-emerald-300/15 dark:bg-emerald-500/10 rounded-full blur-3xl" />
        </div>
                ...
                <AnimatePresence>
                  {isHovered && (
                    <motion.div
                      ...
                      className="absolute z-20 top-full mt-3 bg-slate-900 dark:bg-slate-800 text-white rounded-xl px-4 py-3 text-xs whitespace-nowrap shadow-2xl pointer-events-none"
        ...
        <div className="relative mt-6 text-center text-xs text-slate-400 dark:text-slate-500">
          {t("clickHint")}
        </div>
      </div>
    </div>
